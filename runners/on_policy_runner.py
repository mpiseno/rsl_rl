...
class OnPolicyRunner:
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        # Michael - log histogram of init state sampling distribution
        if locs['it'] % 100 == 0:
            motion_command = self.env.unwrapped.command_manager.get_term('motion')
            dist = motion_command._sampling_probabilities.cpu().numpy()
            self.writer.add_histogram("Metrics/init_state_dist", dist, locs["it"])
